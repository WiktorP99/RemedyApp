...
public class TFLiteObjectDetectionAPIModel implements Detector {
  ...
  private TFLiteObjectDetectionAPIModel(Context context, String modelFilename) throws IOException {
    ...
    optionsBuilder = org.tensorflow.lite.task.vision.detector.ObjectDetector.ObjectDetectorOptions.builder().setMaxResults(NUM_DETECTIONS);
    objectDetector = org.tensorflow.lite.task.vision.detector.ObjectDetector.createFromBufferAndOptions(modelBuffer, optionsBuilder.build());
  }
  ...
  public List<Recognition> recognizeImage(final Bitmap bitmap) {
    ...
    for (Detection detection : results) {
      recognitions.add(
          new Recognition(
              "" + cnt++,
              TranslateLabel(detection.getCategories().get(0).getLabel()),
  ...
  public String TranslateLabel(String label)
  {
    ...
    {
      case "cooling_fluid":
        return "Płyn chłodniczy";
      case "brakes_fluid":
        return "Płyn hamulcowy";
      case "oil_level":
        return "Poziom oleju";
      case "oil_refill":
        return "Wlew oleju";
      case "windshield_washer":
        return "Płyn do wycieraczek";
      default:
        return label;
    }
  }
